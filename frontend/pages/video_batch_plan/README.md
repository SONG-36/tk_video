...
- 每条视频的 `hook_type`、`first_3_seconds_text`、`scroll_stop_reason`、`emotional_trigger`、`viral_pattern_ref` 和 `adapted_pattern_ref`。
- `batch_goal` 可测试不同爆款模式、情绪触发、前 3 秒机制和本地化迁移；同批次不应让 3 条视频使用同一种前 3 秒机制。
...
第 15、16 章的 B/C 级精简输出与第 21 章每条视频完整资产要求的适用关系尚未裁决。本页面保留该约束，不改变任何等级的资产要求。
