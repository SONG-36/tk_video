...
在唯一目标型号上下文中规划 3—5 条差异化视频及 A/B/C 生成等级。
...
`rules/video_batch_rules.json`、`rules/video_type_rules.json`、`rules/material_check_rules.json`、`rules/image_asset_rules.json`、`rules/creative_translation_rules.json`。
...
不得默认五条全 A、把推荐分型数当生成数、生成同脚本变体、混入其他型号或绕过素材限制。

## 后续实现边界

未来只实现结构化规划，不生成脚本或提示词。B/C 精简输出与第 19 章完整资产要求属于设计未裁决约束，本模块不得处理。
