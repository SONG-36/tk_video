...
- 冲突位置：`00_design.md` 第 17 章“统一输出目录结构”与同章“本地项目结构”示例。
- 冲突内容：第 17 章正文规定只有素材充足、批次检查通过且实际规划存在时才允许 `video_04/`、`video_05/`，但旧“本地项目结构”示例直接列出了两个目录，容易被误解为固定或可因目录存在而读取。
- 关联章节：第 7、15、17、18、19 章。
