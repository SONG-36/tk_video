# 质量控制

> 本文档来源于 `00_design.md`，仅用于阅读，不是权威源；如与 `00_design.md` 冲突，以 `00_design.md` 为准。
>
> 来源章节：第 17、18、21 章。

## 系统禁止行为

- 类目未确认时不能生成报告。
- 商品画像未补齐时不能生成视频分型。
- 判断报告未确认时不能导出任务包。
- 核心材料缺失时不能直接生成脚本任务。
- 图片不足时不能进入高质量 AI 视频提示词生成。
- 创意方向未确认时不能生成镜头资产。
- 视频批次未规划时不能生成镜头资产。
- 镜头资产未规划时不能导出任务包。
- 不能让 Codex 编造商品参数或认证信息。
- 不能把所有视频生成成同一种结构。
- 不能默认开箱是主力视频、默认五条全 A 级，或让五条只是同一脚本换说法。

## Codex 输出禁止行为

- 不得把视频分型推荐数量当作实际生成数量。
- 不得生成未绑定 `variant_id` 的视频批次。
- 缺少参数时必须使用占位符，不得编造吸力、续航、噪音、电池容量等数值。
- 不得夸大功效或编写无法拍摄的镜头。
- 不得忽略类目打法、视频分型、镜头顺序或画面内容。
- 不得改变产品颜色、结构、按钮、配件，或让不同镜头的产品外观不一致。
- 不得使用未经授权的品牌、明星、影视角色或明显版权元素。
- 不得把视频写成传统广告片，不得以前 3 秒品牌自我介绍或“今天介绍一款产品”开场。
- 不得直接搬运或翻译国内短剧台词，不得复制爆款视频人物、画面、音乐、品牌或剧情细节。
- 不得让剧情压过产品；产品首次清晰出现不得晚于 8 秒，产品证明不得晚于 12 秒开始。

## 输出验收标准

每条视频必须包含：视频编号、视频分型、创意风格、视频目标、开头钩子、Creative Board、Storyboard、镜头卡、镜头级 Seedance Prompt、图片输入计划、剪辑合成计划、缺失素材、风险提醒和可替换变量。

每条正式完整视频资产还必须具备 hook、proof 和 CTA，并展示 `script_structure`、`retention_design`、`tiktok_native_style`、viral pattern 迁移、文字叠加及声音/口播计划。缺少 hook、proof 或 CTA 不能视为正式完整视频资产。

每个镜头必须包含：`shot_id`、`duration`、`input_images`、`reference_videos`、`main_subject`、`scene`、`action`、`camera_movement`、`lighting`、`style`、`composition`、`product_constraints`、`subtitle_text`、`negative_prompt`、`output_goal`。

## 读取验收与前端展示

- 前端必须如实展示既有质量字段、缺失项和风险项，不得隐藏或改写。
- 未规划视频不得作为有效视频展示；目录或文件存在不能替代已确认批次计划。
- 缺失文件不得显示为正常，也不得由读取器自动生成、修复、重命名或复制。
- 读取器只传递既有质量结论，不修改结论、不新增评分或通过标准。
- 第 15、16 章 B/C 级与第 21 章完整资产要求的关系保持未裁决，读取器和前端不得隐藏或自行裁决。
