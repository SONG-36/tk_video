...
>
> 来源章节：第 4—9、12—19、23、24 章，核心来源为第 17—19 章。
...
任务包还必须携带 TikTok 原生短视频结构与迁移上下文：`hook_type`、`first_3_seconds_text`、`first_3_seconds_visual`、`scroll_stop_reason`、`script_structure`、`retention_design`、`key_message`、`proof_point`、`cta`、`tiktok_native_style`、`text_overlay_plan`、`sound_or_voiceover_plan`、使用到的 `viral_pattern_card` 和生成的 `product_adapted_pattern`。

迁移约束必须明确：Codex 参考的是模式、情绪触发和节奏结构，不是照抄原视频；禁止复制原人物、台词、画面、音乐、品牌或剧情细节。产品必须在前 8 秒内清晰出现，产品证明必须在 12 秒内开始，剧情不能压过产品；跨视频/镜头时序和平衡由后续 validators 校验。
...
- 不得把所有视频写成同一种结构。
- 不得写成传统广告片或以前 3 秒品牌自我介绍开场。
- 产品必须在前 8 秒内清晰出现，产品证明必须在 12 秒内开始，剧情不能压过产品。
...
参考模式而非照抄来源；禁止复制原人物、台词、画面、音乐、品牌或剧情细节。
...
- 视频目标
- 开头钩子
- hook_type、first_3_seconds_text、first_3_seconds_visual、scroll_stop_reason
- script_structure、retention_design、key_message、proof_point、CTA
- tiktok_native_style、text_overlay_plan、sound_or_voiceover_plan
- 使用到的 viral_pattern_card 与 product_adapted_pattern
- Creative Board
- Script
- Storyboard
- 镜头数量
...
- 输出内容按 `00_design.md` 第 21 章验收；已知等级适用关系未裁决时不得自行扩大或缩小要求。
````
...
| `tiktok_creative_requirements` | TikTok 原生结构、逐视频 hook/proof/CTA 和时序要求 |
| `viral_pattern_cards` | 本批次引用的结构化模式卡 |
| `product_adapted_patterns` | 绑定具体视频和目标型号的适配结果 |
| `creative_transfer_constraints` | 跨文化、产品适配、8/12 秒与剧情平衡约束 |
| `blocked_copying_behaviors` | 禁止复制的人物、台词、画面、音乐、品牌和剧情细节 |
...
第 15、16 章的 B/C 级精简输出与第 21 章“每条视频必须包含”完整资产要求的适用关系尚未裁决。模板仅保留两侧要求，不擅自决定 B/C 级的增删范围。
