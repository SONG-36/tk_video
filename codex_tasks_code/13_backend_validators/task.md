...
## 必须读取的文件
- `00_design.md` 第 5—9、15—21、23、24 章；全部 Schema、rules 和 README；`backend/app/validators/README.md`；设计问题和既有测试说明。
## 允许修改的文件
...
- 校验正式视频 hook/proof/CTA 完整性、产品首次清晰出现不晚于 8 秒、产品证明不晚于 12 秒开始、剧情不能压过产品，以及任务包禁止复制来源人物/台词/画面/音乐/品牌/剧情细节约束。
- 错误必须含对象、字段路径、规则来源和可读原因；不得修改输入或调用业务模块。
## 禁止越界事项
- 不生成报告，不做类目/分类/分型/材料/图片/创意/批次/镜头判断，不调用 AI。
## 已知未裁决问题处理方式
- B/C、画像粒度、缺失 category Schema、类目算法、创意映射、材料字段缺口、跨镜头时间轴算法和剧情/产品平衡判定只能返回 `unresolved`/阻断信息，不编码未定义结论。8/12 秒常量本身来自权威设计，但计算方式不得自行发明。
## 验证命令或人工验收方式
